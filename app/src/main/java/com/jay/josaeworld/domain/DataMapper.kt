...
fun AfSearchResponse.toBroadInfo(params: GetMemberUseCase.Params): BroadInfo =
    run {
        ...
        val okCnt =
            if (activeNo == 0) {
                station.upd.today0_ok_cnt
                    .toString()
                    .goodString()
            } else {
                station.upd.today1_ok_cnt
                    .toString()
                    .goodString()
            }
        ...
        incFanCnt =
            if (incFanCnt.toInt() < 0) {
                "-" + incFanCnt.slice(1 until incFanCnt.length).goodString()
            } else {
                incFanCnt.goodString()
            }

        Log.d(
            "DataMapper",
            "GetJson() $streamerName $title $allviewers $imgurl $fanCnt $okCnt $incFanCnt",
        )
...
fun HashMap<*, *>.goodStreamerData(
    ...
    BroadInfo(
        ...
        okCnt = get("okCnt") as? String ?: "0",
        incFanCnt = get("incFanCnt") as? String ?: "0",
        profilePhoto =
            (get("profilePhoto") as? String)
                ?: defaultLogoImgUrl,
        balloninfo = b ?: BallonInfo(),
    )

fun Array<ArrayList<BroadInfo>>.sortedStreamerList(): Array<ArrayList<BroadInfo>> =
    sortedWith(
        compareBy({ teamData ->
            -teamData.sumOf { view ->
